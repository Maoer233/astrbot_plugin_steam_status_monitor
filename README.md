...
## 更新记录
- 2026/09/03 开发更新（相对 V4.3.0）
  - **会话重构**：用 `SessionService` 独占一局游戏的开始/确认退出/关闭；检测循环只投递状态快照。切游戏立即结算上一局；同一游戏 180 秒内假退出才进入确认退出，到期由主轮询 `tick_due` 关闭。运行时不再持有 pending / delayed task / `start_play_times`。
