...
- V4.8.0（2026/09/13）
  ...
  - **并发安全修复**（#51，感谢 @OLRainM）：为 `SessionService` 添加 per-player `asyncio.Lock`，修复游玩时长通知消息重复问题。根因是 `tick_due` 和 `handle` 缺少并发保护，主轮询和 Steam API 超时期间的 tick 可能同时处理同一会话，导致多次添加结束通知。`tick_due` 改为 async 方法，在锁内双重检查会话状态，确保操作串行化。
