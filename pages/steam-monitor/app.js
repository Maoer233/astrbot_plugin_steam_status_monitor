...
async function renderDashboard(){
  ...
  c.innerHTML=`<div class="flex-between mb-20"><h2 class="page-title">仪表盘</h2>
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card"><div class="stat-value">${data.total_bindings}</div><div class="stat-label">账号绑定</div></div>
    </div>
...
async function renderGroups(){
  ...
  const c=document.getElementById("content");
  c.innerHTML=`<div class="flex-between mb-20"><h2 class="page-title">群聊管理</h2><div class="flex gap-8"><span style="color:var(--text-muted)">${gids.length}个群</span><button class="btn btn-sm" onclick="refreshGroupNames()">🔄 刷新群名</button><button class="btn btn-primary btn-sm" onclick="addGroup()">+添加群聊</button></div></div>
    ...
    <div class="modal-overlay" id="batch-modal"><div class="modal" style="max-width:560px"><div class="modal-title">批量导入</div>
      <p style="font-size:12px;color:var(--text-muted);margin-bottom:8px">每行一条：SteamID/链接  QQ号或用户 OpenID  备注名（后两项可选，支持 # 注释）</p>
    ...
    <div class="modal-overlay" id="addsid-modal"><div class="modal" style="max-width:400px"><div class="modal-title" id="addsid-title"></div>
      ...
      <div class="form-group"><label class="form-label">QQ号 / 用户 OpenID（可选）</label><input id="addsid-qq" class="form-input"></div>
    ...
    <div class="modal-overlay" id="bind-modal"><div class="modal" style="max-width:360px"><div class="modal-title">绑定账号</div>
      <div class="form-group"><label class="form-label">QQ号 / 用户 OpenID</label><input id="bind-qq" class="form-input"></div>
  ...
  let sel=gids[0]||null;
  function renderList(){const l=document.getElementById("group-list");l.innerHTML=gids.map(gid=>{const gname=groupNames[gid]||`群${gid}`;const displayName=gname===gid?gname:`<div style="font-weight:500">${escapeHtml(gname)}</div><div style="font-size:11px;color:var(--text-muted)">${escapeHtml(gid)}</div>`;return`<div class="group-list-item${gid===sel?" active":""}" style="display:flex;justify-content:space-between;align-items:center" onclick="selG(${jsArg(gid)})"><div style="flex:1">${displayName}<span style="color:var(--text-muted);margin-left:8px">(${groups[gid].length}人)</span></div><button class="btn btn-danger btn-sm" style="padding:2px 6px;font-size:11px" onclick="event.stopPropagation();delGroup(${jsArg(gid)})">✕</button></div>`}).join("")}
  ...
  async function renderDetail(gid){
    ...
    const gname=groupNames[gid]||`群${gid}`;
    let h=`<div class="flex-between mb-16"><div><span class="card-title" style="margin-bottom:0">${escapeHtml(gname)}</span>${gname!==`群${gid}`&&gname!==gid?`<div style="font-size:12px;color:var(--text-muted);margin-top:4px">OpenID: ${escapeHtml(gid)}</div>`:``}</div><div class="flex gap-8"><button class="btn btn-primary btn-sm" onclick="showAddSIDModal(${jsArg(gid)})">+添加</button><button class="btn btn-sm" onclick="showBatchModal(${jsArg(gid)})">📋批量导入</button></div></div>`;
    if(!ps.length){h+='<div class="empty-state"><span class="mdi mdi-account-off"></span><p>暂无玩家</p></div>';d.innerHTML=h;return}
    h+='<table class="table"><thead><tr><th></th><th>玩家</th><th style="width:140px">绑定账号</th><th>备注</th><th>状态</th><th>操作</th></tr></thead><tbody>';
...
window.refreshGroupNames=async()=>{try{const r=await API.get("/api/groups/names");groupNames=r.group_names||{};toast("群名称已刷新");navigateTo("groups")}catch(e){toast("刷新失败: "+e.message,"error")}};
window.addGroup=async()=>{const g=await pagePrompt("添加群聊","群号或官方群 OpenID");if(!g)return;const r=await API.post("/api/groups/add-group",{group_id:g});if(r.ok){toast("已添加");navigateTo("groups")}else toast(r.error||"失败","error")};
...
const SL={steam_api_key:"Steam Web API密钥",sgdb_api_key:"SteamGridDB API密钥",steam_api_base:"Steam Web API代理地址",steam_store_base:"Steam商店代理地址",sgdb_api_base:"SteamGridDB代理地址",fixed_poll_interval:"固定轮询间隔(秒)",smart_poll_intervals:"智能轮询间隔(分)",retry_times:"API重试次数",max_group_size:"单群最大监控人数",detailed_poll_log:"详细轮询日志",enable_achievement_poll:"成就轮询推送",enable_steam_style:"列表新版渲染风格",enable_game_start_notify:"游戏开始通知",enable_game_end_notify:"游戏结束通知",enable_network_fluctuation_notify:"网络波动通知",notify_send_image:"通知发送图片",notify_send_text:"通知发送文本",game_filter_mode:"游戏过滤模式",game_filter_ids:"游戏过滤游戏ID",rank_push_hour:"排行榜推送小时",rank_push_minute:"排行榜推送分钟",enable_proxy:"启用网络代理",proxy_url:"代理链接",cache_avatar_hours:"头像缓存时间(小时)",cache_avatar_frame_hours:"头像框缓存时间(小时)"};
const SO=["steam_api_key","sgdb_api_key","steam_api_base","steam_store_base","sgdb_api_base","fixed_poll_interval","smart_poll_intervals","retry_times","max_group_size","enable_game_start_notify","enable_game_end_notify","enable_network_fluctuation_notify","enable_achievement_poll","enable_steam_style","notify_send_text","notify_send_image","detailed_poll_log","game_filter_mode","game_filter_ids","rank_push_hour","rank_push_minute","enable_proxy","proxy_url","cache_avatar_hours","cache_avatar_frame_hours"];
